...
$pergunta = $_SESSION['questions'][$current_question]['question'];
...
<div class="container">
    ...
    <hr>
    <!-- Exibe a pergunta, que é a pergunta, para o qual o usuário deve escolher a resposta -->
    <h4><strong><?= $pergunta ?></strong></h4>
    <hr>

    <div>
        <!-- Exibe as opções de respostas, que são capital de diferentes países -->
        <h3 style="cursor: pointer" id="answer_0"><?= $database[$answers[0]][1] ?></h3>
        <h3 style="cursor: pointer" id="answer_1"><?= $database[$answers[1]][1] ?></h3>
        <h3 style="cursor: pointer" id="answer_2"><?= $database[$answers[2]][1] ?></h3>
    </div>
