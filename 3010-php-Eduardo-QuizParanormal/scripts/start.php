<?php
    ...
    function prepare_game($total_questions) {
        ...
        // Acessa a variável global $database, que contém os dados das perguntas e respostas.
        global $database;
        ...
        while (count($ids) < $total_questions) {
            // Gera um ID aleatório dentro do intervalo de índices do array $database.
            $id = rand(0, count($database) -1);
            // Verifica se o ID já foi selecionado. Se nao, adiciona ao array $ids.
            if (!in_array($id, $ids)) {
                $ids[] = $id;
            }
        }
        ...
        foreach($ids as $id){
            ...
            $answers = [];
            // A primeira resposta é sempre o ID correto (resposta correspondente a pergunta);
            $answers[] = $id;
            // Preenche as outras respostas com IDs aleatórios que nção sejam iguais ao correto.
            while(count($answers) < 3) {
                $tmp = rand(0, count($database) -1);
                // Verifica se a resposta gerada já oi selecionada. Se não, adiciona à lista de respostas
            ...
            $questions[] = [
                'question' => $database[$id][0], // A pergunta
                'correct_answer' => $id, // O ID da resposta correta.
        ...
        // Inicializa os dados do jogo na sessão, incluindo o número total de questões, o número da questão atual e as contagens de respostas corretas e erradas.
        $_SESSION['game'] = [
            'total_questions' => $total_questions, // Numero total de questões
...
<div class="container">
    ...
    <form action="index.php?route=start" method="post">
        <!-- Pergunta sobre o número de questões, com um campo input do tipo number -->
        <h3>
            <label for="text_total_questions" class="form_label">Número de questões:</label>
        <!-- O valor inicial 10, o mínimo é 1 e o máximo é 12. -->
         <input type="number" class="form-control" id="text_total_questions" name="text_total_questions" value="10" min="1" max="12">    
    </h3>
        <!-- Botão para enviar o formulário e iniciar o jogo -->
         <div>
